...
class UserController extends BaseController
{
    ...
    public function doCheckIn($request, $response, $args)
    {
        ...
        $invite_num = rand(Config::get('checkinMin'), Config::get('checkinMax'));
        $invite_freq = intval(Config::get('checkinFrequency'));
        ...
        if ($this->user->checkinTimesThisMonth() % $invite_freq === 0) {
            $this->user->invite_num = $this->user->invite_num + $invite_num;
            $res['msg'] = sprintf("获得了 %d 邀请码.", $invite_num);
        } else {
            $res['msg'] = '签到成功';
        }
        $this->user->save();
